...
export default function App() {
  ...
  // Team
  const submitMyPicks = async () => { if (!activeSong || !clips.length) return; await dbSubmitPicks(activeSong, user.name, clips.map(c => ({ startTime: c.startTime, endTime: c.endTime, notes: c.notes || "", dur: c.dur }))); setSubmitted(true); flash("Picks submitted!"); };
        ...
        {page === "submit" && !isLeader && <div>
          ...
          {hasAudio && <div>
            ...
            <div style={{ display: "grid", gap: 6, marginBottom: 16 }}>{clips.map((c, idx) => <div key={c.id} onClick={() => setSel(idx)} style={{ ...cs(idx === sel), display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input type="text" placeholder="Note..." value={c.notes || ""} onChange={e => { e.stopPropagation(); updateNote(idx, e.target.value); }} onClick={e => e.stopPropagation()} style={{ width: 120, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 4, padding: "3px 6px", color: "#ccc", fontSize: 9, outline: "none" }} />
              ...
              <button onClick={e => { e.stopPropagation(); delClip(idx); }} style={{ background: "rgba(255,51,102,0.05)", border: "1px solid rgba(255,51,102,0.1)", color: "#ff3366", fontSize: 8, padding: "3px 6px", borderRadius: 4, cursor: "pointer" }}>✕</button>
            </div>)}</div>
            {submitted && <div style={{ background: "rgba(68,255,136,0.08)", border: "1px solid rgba(68,255,136,0.2)", borderRadius: 8, padding: "10px 14px", textAlign: "center", color: "#44ff88", fontSize: 13, fontWeight: 600, marginBottom: 8 }}>✓ Submitted! You can update anytime.</div>}
            <button onClick={submitMyPicks} disabled={clips.length === 0} style={{ ...bs(clips.length > 0), padding: "10px 24px", fontSize: 13, fontWeight: 600, opacity: clips.length === 0 ? 0.4 : 1, width: "100%" }}>{submitted ? "Update Submission" : `Submit ${clips.length} Pick${clips.length !== 1 ? "s" : ""}`}</button>
          </div>}
        </div>}
      </div>
