      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "20px 12px 80px", width: "calc(100% - 0px)", overflowX: "hidden", overflowY: "auto" }}>
        ...
        {page === "review" && isLeader && <div>
          ...
          {clips.length > 0 && <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gap: 7 }}>{clips.map((c, idx) => { let bestLabel = null, bestDiff = Infinity; consensus.forEach((cons, ci) => { const diff = Math.abs(c.startTime - cons.startTime); if (diff <= 3 && diff < bestDiff) { bestLabel = `C${ci + 1}`; bestDiff = diff; } }); return <ClipCard key={c.id || idx} c={c} idx={idx} sel={sel} playing={playing} playingClip={playingClipIdx} isModified={isModified(c)} bs={bs} onSel={setSel} onPlay={playClip} onExport={expClip} onDur={setClipDur} onNote={updateNote} onAB={startAB} onRevert={revertClip} onDel={delClip} editNote={editNote} setEditNote={setEditNote} ab={ab} clips={clips} consensusLabel={bestLabel} />; })}</div>
          </div>}
